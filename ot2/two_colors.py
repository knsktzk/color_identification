...
def run(protocol: protocol_api.ProtocolContext):
    ...
    split_well_ls = list(np.array_split(well_ls, 24))
    ...
    def dispence(pool, wells, mix_flag=False):
        ...
        for well in wells:
            pipetto.dispense( 60, plate[well])

        if mix_flag:
            for well in reversed(wells):
                pipetto.mix(2, 100, plate[well])
    ...
    for well_ls in split_well_ls:
        ...
        if pool_num % 2 == 0:
            pipetto.pick_up_tip()
            dispence(color_2_pools[0], well_ls, True)
            pipetto.drop_tip()

            pool_num +=1
        else:
            pipetto.pick_up_tip()
            dispence(color_2_pools[1], well_ls, True)
            pipetto.drop_tip()
